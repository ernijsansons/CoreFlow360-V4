...
aiMonitoringRoutes.get('/dashboard', async (c) => {
  try {
    ...
    const activeAudits = scheduledAudits.filter(a => a.enabled).length;
    const runningExecutions = recentHistory.filter(h => h.status === 'running').length;
    const recentFailures = recentHistory.filter(h =>
      h.status === 'failed' &&
    ...
    // Calculate trends
    const last24h = recentHistory.filter(h =>
      h.startTime > new Date(Date.now() - 24 * 60 * 60 * 1000)
    );
      ...
      dashboard: {
        ...
        metrics: {
          ...
          successRate: last24h.length > 0 ?
            Math.round((last24h.filter(h => h.status === 'completed').length / last24h.length) * 100) : 100,
...
aiMonitoringRoutes.get('/schedules', async (c) => {
  try {
    ...
    const enrichedAudits = await Promise.all(
      scheduledAudits.map(async (audit) => {
        let pastRuns = [];
        if (includePastRuns) {
    ...
    return c.json({
      ...
      schedules: {
        ...
        summary: {
          ...
          active: scheduledAudits.filter(a => a.enabled).length,
          inactive: scheduledAudits.filter(a => !a.enabled).length,
...
aiMonitoringRoutes.post('/schedules', async (c) => {
  try {
...
aiMonitoringRoutes.put('/schedules/:auditId', async (c) => {
  try {
...
aiMonitoringRoutes.delete('/schedules/:auditId', async (c) => {
  try {
    const auditId = c.req.param('auditId');
...
aiMonitoringRoutes.get('/executions', async (c) => {
  try {
    ...
    const stats = {
      ...
      averageExecutionTime: executions
        .filter(e => e.duration)
        .reduce((sum, e) => sum + e.duration!, 0) / Math.max(executions.filter(e => e.duration).length, 1),
      averageScore: executions
        .filter(e => e.result?.score)
        .reduce((sum, e) => sum + e.result.score, 0) / Math.max(executions.filter(e => e.result?.score).length, 1)
    };
...
aiMonitoringRoutes.get('/alerts', async (c) => {
  try {
    ...
    const stats = {
      ...
      byType: {
        performance: allAlerts.filter(a => a.type === 'performance' && !a.resolved).length,
        availability: allAlerts.filter(a => a.type === 'availability' && !a.resolved).length,
        quality: allAlerts.filter(a => a.type === 'quality' && !a.resolved).length,
        security: allAlerts.filter(a => a.type === 'security' && !a.resolved).length,
        cost: allAlerts.filter(a => a.type === 'cost' && !a.resolved).length
      }
    };
...
// Resolve Alert
aiMonitoringRoutes.post('/alerts/:alertId/resolve', async (c) => {
  try {
...
aiMonitoringRoutes.get('/metrics/realtime', async (c) => {
  try {
    const metrics = {
      timestamp: new Date().toISOString(),
      system: {
...
aiMonitoringRoutes.get('/health', async (c) => {
  try {
    const health = {
      status: 'healthy',
      timestamp: new Date().toISOString(),
      ...
      metrics: {
        ...
        activeAlerts: scheduler ? (await scheduler.getMonitoringAlerts()).filter(a => !a.resolved).length : 0,
