...
export const InvoiceCreationCapability: CapabilitySpec = {/
  ...
  id: 'invoice:create',;"
  name: 'Create Invoice',;"
  description: 'Creates a new invoice with line items, tax;"
  calculations, and payment terms. Validates all business rules and maintains audit trail.',;"
  version: '1.2.0',;"
  category: 'database',
;/
...
export const LedgerPostingCapability: CapabilitySpec = {/
  ...
  description: 'Posts double-entry accounting transactions to;"
  the general ledger with automatic validation and balancing checks.',;"
  version: '1.1.0',;"
  category: 'database',
;/
  // Parameters;
...
;
  aiConfiguration: {"
    promptTemplate: 'Process payment of {amount}"
  {currency} for customer {customerId} using {paymentMethod}. Validate against invoices: {invoiceIds}',;"
    fallbackBehavior: 'error',;"
...
/
    ...
    if (terms.discountPercentage && (typeof terms.discountPercentage !==;"
  'number' || terms.discountPercentage < 0 || terms.discountPercentage > 100)) {
      return false;
    }
...
/
    ...
    const;"`
  expectedPeriod = `${transactionDate.getFullYear()}-${String(transactionDate.getMonth() + 1).padStart(2, '0')}`;
