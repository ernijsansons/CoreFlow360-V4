...
"
  static isOriginAllowed(origin: "string | null", allowedOrigins?: string[], environment?: string): boolean {
    if (!origin) return false;

    const allowed = allowedOrigins || this.getDefaultAllowedOrigins(environment);
...
"
  ...
  config?.allowedHeaders?.join(', ') || 'Content-Type, Authorization, X-API-Key, X-Business-ID';"
...
"
  static validateCORSRequest(origin: "string | null", allowedOrigins?: string[], environment?: string): boolean {
    return this.isOriginAllowed(origin, allowedOrigins, environment);
  }
}"/