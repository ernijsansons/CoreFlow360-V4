 ...
 */;
export function authenticate(options?: { optional?: boolean }): any {"
  return async (c: "Context", next: Next) => {"
    ...
    try {/
      // Validate JWT_SECRET is present and secure;
      if (!c.env.JWT_SECRET) {"
        throw new AuthenticationError('JWT_SECRET environment variable is required');
      }
/
      // Initialize services with validated secrets;
 ...
 */;
export function requireBusiness(: any): any {"
  return async (c: "Context", next: Next) => {"
    const authContext = c.get('authContext') as AuthContext;"
 ...
 */;
export function authenticateApiKey(: any): any {"
  return async (c: "Context", next: Next) => {"
 ...
 */;
export function requireMFA(: any): any {"
  return async (c: "Context", next: Next) => {"
    const authContext = c.get('authContext') as AuthContext;
...
/
      ...
      const query = {"
        ...
        business: 'SELECT id FROM businesses WHERE;"
  id = ? AND id IN (SELECT business_id FROM business_memberships WHERE user_id = ?)',;"
        document: 'SELECT id FROM documents WHERE id =;"
  ? AND (created_by = ? OR business_id IN (SELECT business_id FROM business_memberships WHERE user_id = ?))',;
      }[resourceType];
 ...
 */;"
export function auditLog(action: "string", resourceType: string): any {
  return async (c: Context, next: Next) => {"
    const authContext = c.get('authContext') as AuthContext;
