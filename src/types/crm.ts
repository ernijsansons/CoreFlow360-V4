...
// AI Task Payload for Voicemail
export interface LeaveVoicemailTaskPayload {
  lead_id: string;
  call_id?: string;
  attempt_number: number;
  voicemail_type: VoicemailType;
...
export interface TranscriptSegment {
  ...
  entities?: {
    name: string;
    type: 'person' | 'company' | 'product' | 'competitor' | 'location';
    confidence: number;
  }[];
}
...
export interface Participant {
  id: string;
  name: string;
  role: 'sales_rep' | 'prospect' | 'customer' | 'champion' | 'decision_maker' | 'gatekeeper';
  email?: string;
  title?: string;
  company?: string;
  speakingTime?: number;
...
export interface CompetitorAnalysis {
  ...
  threats: {
    competitor: string;
    threat: 'price' | 'features' | 'relationship' | 'timing' | 'reputation';
...
export interface SpeakingRatio {
  ...
  prospect: number;
  ideal: number;
  assessment: 'too_much_talking' | 'good_balance' | 'not_talking_enough';
...
export interface QuestionAnalysis {
  ...
  byType: {
    type: 'open' | 'closed' | 'discovery' | 'confirmation' | 'objection_handling';
    count: number;
    examples: string[];
  }[];
...
export interface MonologueAnalysis {
  instances: {
    ...
    startTime: number;
    content: string;
    assessment: 'good' | 'acceptable' | 'too_long';
  }[];
  totalMonologueTime: number;
...
export interface EmotionalState {
  emotion: 'excited' | 'interested' | 'neutral' | 'concerned' | 'frustrated' | 'confused';
  intensity: number;
  timestamp: number;
  triggers?: string[];
}
...
export interface CoachingAlert {
  ...
  timestamp?: number;
  priority: 'low' | 'medium' | 'high';
  category: 'talk_time' | 'questions' | 'objection' | 'next_steps' | 'energy' | 'discovery';
}

export interface CoachingRecommendation {
  category: 'discovery' | 'presentation' | 'objection_handling' | 'closing' | 'rapport' | 'next_steps';
  recommendation: string;
  rationale: string;
...
export interface RealTimeCoaching {
  ...
  suggestions: {
    message: string;
    timing: 'now' | 'next_pause' | 'next_question';
    type: 'question' | 'statement' | 'objection_response';
  }[];
  talkTimeAlert?: {
    currentRatio: number;
    targetRatio: number;
...
export interface CallOutcome {
  result: 'won' | 'lost' | 'advance' | 'no_decision' | 'follow_up';
  probability: number;
  nextSteps: string[];
  timeline: string;
  risks: string[];
...
export interface CallStream {
  ...
  isActive: boolean;
  platform: 'zoom' | 'teams' | 'phone' | 'meet' | 'other';
...
export interface Situation {
  type: 'objection' | 'competitor_mention' | 'pricing_discussion' | 'long_monologue'
  | 'buying_signal' | 'pain_point' | 'feature_request' | 'next_steps' | 'closing_opportunity';
  confidence: number;
  timestamp: number;
  context: string;
  severity: 'low' | 'medium' | 'high';
...
export interface Guidance {
  type: 'objection_handling' | 'competitive' | 'pricing' | 'discovery' | 'closing' | 'rapport';
  title: string;
  ...
  doNotSay: string[];
  suggestedPhrases: string[];
  urgency: 'immediate' | 'next_pause' | 'when_appropriate';
  effectiveness: number;
}
...
export interface PricingGuidance {
  situation: string;
  approach: 'value_first' | 'comparison' | 'roi_focus' | 'payment_terms' | 'discount_authority';
...
export interface CoachingTip {
  type: 'talk_time' | 'energy' | 'questions' | 'next_steps' | 'rapport' | 'discovery';
  message: string;
...
export interface LiveCoachingMessage {
  id: string;
  type: 'guidance' | 'battlecard' | 'pricing' | 'tip' | 'alert' | 'suggestion';
  timestamp: number;
  priority: 'low' | 'medium' | 'high' | 'critical';
  content: Guidance | Battlecard | PricingGuidance | CoachingTip | string;
  displayDuration?: number; // seconds to show message
...
// Self-Improving AI Interfaces
export interface Interaction {
  id: string;
  type: 'email' | 'call' | 'linkedin' | 'sms' | 'whatsapp' | 'meeting';
  leadId: string;
  strategy: string;
  prompt?: string;
  variant?: string;
...
export interface Outcome {
  ...
  success: boolean;
  result: 'response' | 'meeting_booked' | 'deal_advanced' | 'objection' | 'no_response' | 'unsubscribe' | 'bounce';
...
export interface Strategy {
  id: string;
  name: string;
  type: 'outreach' | 'followup' | 'nurture' | 'objection_handling' | 'closing';
  description: string;
...
export interface Pattern {
  ...
  applicability: string[]; // Which segments/scenarios this applies to
  discovered: string;
...
export interface CustomerSegment {
  ...
  strategies: string[]; // Strategy IDs that work for this segment
  patterns: string[]; // Pattern IDs relevant to this segment
...
export interface Playbook {
  ...
  version: number;
  sections: {
    idealCustomerProfile: PlaybookSection;
    qualifyingQuestions: PlaybookSection;
    discoveryStructure: PlaybookSection;
    commonObjections: PlaybookSection;
    proofPoints: PlaybookSection;
    emailTemplates: PlaybookSection;
    callScripts: PlaybookSection;
    competitivePositioning: PlaybookSection;
    pricingGuidance: PlaybookSection;
    closeTechniques: PlaybookSection;
  };
  performance: {
    adoptionRate: number;
    winRate: number;
    averageDealSize: number;
    salesCycle: number;
    userFeedback: number; // 0-5 rating
  };
  metadata: {
    generatedBy: 'ai' | 'human' | 'hybrid';
    dataPoints: number; // Number of deals used to generate
    lastAnalysis: string;
    confidence: number;
  };
  active: boolean;
...
export interface Feedback {
  ...
  strategyId?: string;
  interactionId?: string;
  type: 'effectiveness' | 'accuracy' | 'usability' | 'outcome' | 'suggestion';
  rating: number; // 1-5
  comment: string;
  category: string;
  suggestions?: string[];
...
export interface LearningMetrics {
  ...
  improvements: {
    ...
    playbookUpdates: number;
  };
  performance: {
    overallWinRate: number;
    responseRateImprovement: number;
    dealVelocityImprovement: number;
    revenueImpact: number;
  };
  experiments: {
    active: number;
...
export interface ExperimentResult {
  id: string;
  experimentType: 'strategy' | 'prompt' | 'timing' | 'channel' | 'content';
  hypothesis: string;
  variants: {
    control: any;
    test: any;
  ...
  decision: 'adopt' | 'reject' | 'iterate' | 'expand';
  impact: string;
  startDate: string;
  endDate: string;
}
...
export interface StrategyUpdate {
  strategyId: string;
  type: 'performance_adjustment' | 'prompt_optimization' | 'targeting_refinement' | 'channel_rebalancing';
  changes: {
    field: string;
    oldValue: any;
    newValue: any;
